...
function ExamPageContent({
    ...
    if (wrongQuestions.length > 0) {
      for (let i = 0; i < questions.length; i++) {
        if (!wrongQuestions.includes(questions[i].props.question.questionID)) {
          correctQuestionsContainer.push(
            questions[i].props.question.questionID
          );
        }
      }
    } else {
  ...
  const submitAnswers = () => {
    ...
    writtenAnswers = Array.from(writtenAnswerContainer).map(
      (answer) => answer.value
    );
    ...
    for (let i = 0; i < writtenAnswers.length; i++) {
      if (
        !correctIndexesContainer.includes(i) &&
        writtenAnswers[i] === correctAnswers[i]
      ) {
          ...
          <div className="popup-container">
            <div className="popups">
              ...
              <div className="middle-popup">
                <div className="p">
                  You are about to submit your answers and calculate your mark.
                  If there are any questions left unanswered, you will not get
                  another chance.
                </div>
              </div>
  ...
  const TogglePopup = (bool) => {
    if (bool) {
      window.scrollTo(0, 0);
      document
        .querySelector(".exams-container")
        .classList.add("exams-container-on-popup");
      setEnablePopup(true);
    } else {
      document
        .querySelector(".exams-container")
        .classList.remove("exams-container-on-popup");
      ...
      let probability =
        Math.round(
          (wrongQuestions.length /
            (wrongQuestions.length + correctQuestions.length)) *
            10
        ) / 10;
  ...
  return (
    <div className="exams-container">
      ...
      <ExamPageContent
        ...
        wrongQuestions={wrongQuestions}
        submitButtonClicked={submitButtonClicked}
        setUserWrittenAnswers={setUserWrittenAnswers}
